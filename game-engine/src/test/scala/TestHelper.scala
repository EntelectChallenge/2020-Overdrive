...
object TestHelper {
    ...
    def initialiseGameWithMultipleDifferentMapObjectsAt(lane: Int, blockNumbers: Array[Int], mapObjectTypes: Array[Int]): GameMap = {
        val testPlayers = new Array[Player](2)
        testPlayers(0) = testPlayer1
        testPlayers(1) = testPlayer2
        val testPlayersJava = testPlayers.toList.asJava
        ...
        var mapObjectCounter = 0;
        var mapObjectMax = mapObjectTypes.length;

        for (blockNumber <- blockNumbers) {
            if (mapObjectCounter >= mapObjectMax) mapObjectCounter = 0;
            var objectToPlace = mapObjectTypes(mapObjectCounter);
            mapObjectCounter+= 1;
            gameMap.asInstanceOf[CarGameMap].placeObjectAt(lane, blockNumber, objectToPlace)
        }
